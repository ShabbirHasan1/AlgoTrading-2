...
token_path = "D:\\AlgoTr\\FXCM_API.txt"
...
def renko_merge(DF):
    ...
    merged_df["macd"]= MACD(merged_df,5,8,3)[0]
    merged_df["macd_sig"]= MACD(merged_df,5,8,3)[1]
...
starttime=time.time()
timeout = time.time() + 60*60*0.25  # 60 seconds times 60 meaning the script will run for 1 hr
